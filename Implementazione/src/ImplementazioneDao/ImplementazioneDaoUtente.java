...
{	
	...
	public int unicitaUtente(String nomeUtente)
	{
		...
		try 
		{
			PreparedStatement queryUnicitaUtente = connection.prepareStatement("SELECT COUNT(*) AS ut FROM Utente WHERE NomeUtente='"+nomeUtente+"'");
			ResultSet rs = queryUnicitaUtente.executeQuery();
			...
			{
				System.out.println("Nome utente gia'� utilizzato.");
			}
		}
		catch(SQLException e)
		{
	...
	public int unicitaMail(String Mail)
	{
		...
		try 
		{
			PreparedStatement queryUnicitaMail = connection.prepareStatement("SELECT COUNT(*) AS ut FROM Utente WHERE Mail='"+Mail+"'");
			ResultSet rs = queryUnicitaMail.executeQuery();
			System.out.println("Procedura unicita' eseguita con successo.");
