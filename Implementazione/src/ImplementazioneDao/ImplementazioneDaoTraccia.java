...
public class ImplementazioneDaoTraccia implements DaoTraccia 
{
	...
	public ArrayList<Traccia> listaTraccia()
	{
		...
		try 
		{
			...
			while(rs.next())
			{
				Traccia t = new Traccia(rs.getInt("IdTraccia"),rs.getString("Titolo"),rs.getInt("Durata"),rs.getInt("Anno"),rs.getString("Genere"),rs.getInt("Versione"),rs.getString("qualitàaudio"),rs.getString("Tipo"),rs.getString("Accesso"),rs.getInt("percentualeprod"));
				lista.add(t);
			}
			rs.close();
		}
	...
	public ArrayList<Traccia> listaPreferiti(int codiceUtente)
	{
		...
		try 
		{
			PreparedStatement queryListaTraccia = connection.prepareStatement("SELECT tr.idtraccia, tr.titolo, tr.durata, tr.anno, tr.genere, tr.versione, tr.qualitàaudio, tr.tipo, tr.accesso, tr.percentualeprod FROM traccia tr, branipreferiti bp WHERE bp.traccia=tr.idtraccia AND bp.utente="+codiceUtente+";");
			ResultSet rs = queryListaTraccia.executeQuery();
