...
{		
	...
	public CancellaAlbumGui(ControllerAlbum controller,JFrame adminGui,Utente utente)
	{
		System.out.println("Ti trovi in CancellaAlbumGui");
		listaAlGui = new JFrame();
		listaAlGui.setBackground(new Color(20, 20, 20));
		listaAlGui.setTitle("Cancella Album");
		...
		listAlbum=controller.listaAlbum();
	   ...
	   JButton cancellaAlbum = new JButton("Cancella Album");
	   cancellaAlbum.setForeground(new Color(20,20,20));
	   cancellaAlbum.setFont(new Font("Courier", Font.CENTER_BASELINE,20));
	   cancellaAlbum.setBounds(280, 630, 250, 45);
	   contentPane.add(cancellaAlbum);
	   ...
	   indietro.addActionListener(new ActionListener() 
	   {
		...
		cancellaAlbum.addMouseListener(new MouseAdapter() 
		{
			public void mouseClicked(MouseEvent e) 
			{
				if(listAlb.getSelectedIndex() != -1)
				{
					album=listAlbum.get(listAlb.getSelectedIndex());
					controller.cancellaAlbum(utente.getNomeUtente(), utente.getPassword(),album.getIdAlbum());
					JOptionPane.showMessageDialog(cancellaAlbum,"Cancellazione avvenuta con successo.");
					listaAlGui.dispose();
					adminGui.setVisible(true);
				}
				else
				{
					JOptionPane.showMessageDialog(cancellaAlbum, "Scegliere un Album.");
				}
			}
